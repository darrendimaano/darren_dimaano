...
<head>
  ...
  <style>
    body { 
      ...
      background: linear-gradient(to right, #0f0f1e, #1a1a2e, #0f0f1e); 
      color: #e5e5e5;
    }
    .font-title { 
    ...
    .glow-header {
      text-shadow: 0 0 8px #00f0ff, 0 0 16px #8a2be2;
    }
    .hp-current {
      padding: 6px 12px;
      border-radius: 6px;
...
<body class="min-h-screen">
  ...
  <nav class="bg-gradient-to-r from-cyan-700 via-indigo-800 to-purple-900 shadow-lg border-b-2 border-cyan-400">
    <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
      <h1 class="text-cyan-200 font-title text-2xl flex items-center gap-2 glow-header">
    ...
    <div class="futuristic-card shadow-xl rounded-xl p-6">
        ...
        <table class="w-full text-center border-collapse">
          ...
          <tbody class="text-gray-200 text-sm">
            <?php if(!empty($users)): ?>
              ...
              <?php endforeach; ?>
            <?php else: ?>
              <tr><td colspan="4" class="py-4 text-gray-400">No students found.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
      ...
      <div class="mt-4 flex justify-between items-center">
        ...
        <a href="<?=site_url('auth/logout');?>"
           class="btn-hover bg-gradient-to-r from-purple-700 to-cyan-600 text-white px-4 py-2 rounded-lg shadow flex items-center gap-2 transition duration-300">
